...
//        + ", CONSTRAINT openOutputs_fk FOREIGN KEY (id) REFERENCES openOutputsIndex(id)"
    ...
    public PostgresFullPrunedBlockStore(NetworkParameters params, String dbName, int fullStoreDepth) throws BlockStoreException {
        ...
        try {
            ...
            log.error("check CLASSPATH for Postgresql jar ", e);
        }
    ...
    private void createTables() throws SQLException, BlockStoreException {
        ...
        log.debug("PostgresFullPrunedBlockStore : CREATE open output index table");
        ...
        try {
            try {
                ...
                if (transactions == null) {
                    s = conn.get().prepareStatement("INSERT INTO undoableBlocks(hash, height, txOutChanges)"
                                                    + " VALUES(?, ?, ?)");
                    s.setBytes(3, txOutChanges);
                } else {
                    s = conn.get().prepareStatement("INSERT INTO undoableBlocks(hash, height, transactions)"
                                                    + " VALUES(?, ?, ?)");
